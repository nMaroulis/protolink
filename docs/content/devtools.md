...
Add nodes from the palette, drag them on the canvas, and select a node to edit it. On desktop, Studio stays within the available viewport: the palette, canvas, and inspector scroll independently rather than making the whole page grow. **Code** and **Logs** open a fixed output drawer whose tabs switch between generated Python, Blueprint JSON, and runtime logs. **Restore starter** returns to the packaged example, while **Clear canvas** creates an empty, persisted workspace and remains undoable.
...
**Generate Python** validates the blueprint and returns a readable module built from Protolink's public APIs, then opens the output drawer on its Python tab. The code view shows warnings for incomplete topology, and the result can be copied or downloaded under its generated filename. Generating does not execute anything. In a served dashboard, **Run** starts the generated module as the dashboard's single active Studio subprocess and opens the drawer on its Logs tab; **Stop** terminates it. Closing the dashboard also stops the child process and removes its temporary script.
