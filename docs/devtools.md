...
The dashboard is deliberately small: no build step, no frontend dependencies, and no telemetry upload. It serves a local page with recent tasks, run reports, registry agents, agent health probes, a chat panel for HTTP LLM agents, and a disabled Studio preview tab. The JSON endpoint at `/api/snapshot` uses the same collector as static rendering.
