...
The dashboard is deliberately small: no build step, no frontend dependencies, and no telemetry upload. It serves a local page with recent tasks, run reports, registry agents, and a disabled Studio preview tab. The JSON endpoint at `/api/snapshot` uses the same collector as static rendering.
...
- Recent task/report tables for quick navigation.
- Registry card summaries.
...
- HTML renderers turn those structures into standalone dashboard pages with the disabled Studio preview included.
...
Because it uses `create_llm("mock")`, it does not need provider credentials. The static dashboard HTML generated by the example also includes the demo registry snapshot.
